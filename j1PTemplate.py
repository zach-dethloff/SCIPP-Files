...
j1PT_val = val[b'j1PT'][big_mask]

back_val = b1[b'j1PT'][back_mask]

back2_val = b2[b'j1PT'][back2_mask]

backsum = np.append(back_val, back2_val)

backweights = np.append(b2[b'weight'][back2_mask], b1[b'weight'][back_mask])

plt.hist(j1PT_val, bins=100, range=(0,1400),
	color='g', label='Signal j1PT', alpha = 0.5,
	weights=val[b'weight'][big_mask])
plt.hist(back_val, bins=100, range=(0,1400),
	color = 'r', label='Background j1PT', alpha=0.5,
	weights=b1[b'weight'][back_mask])
plt.hist(back2_val, bins=100, range=(0,1400),
	color = 'b', label = 'Background j1PT 2',alpha=0.5,
	weights=b2[b'weight'][back2_mask])
plt.hist(backsum, bins = 100, range=(0,1400), 
	color='orange', label = 'Background j1PT sum', alpha = 0.5,
	weights = backweights)
plt.title('j1PT')
plt.yscale('log')
plt.legend()
